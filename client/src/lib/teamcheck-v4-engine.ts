...
function buildTeamOhnePerson(c: Ctx): string {
  ...
  if (c.matchCase === "TOP1_TOP2") {
    ...
    paras.push(`Ohne die Person verliert das Team vor allem den ergänzenden Impuls in ${COMP_DOMAIN[c.personSecondary]}, den die Person zusätzlich zur gemeinsamen Grundlogik (${COMP_SHORT[c.teamPrimary]}) eingebracht hat. Die Grundlogik des Teams bleibt unverändert, aber die besondere Arbeitsweise der Person — ${COMP_SHORT[c.personSecondary]} statt ${COMP_SHORT[c.teamSecondary]} — fehlt als ergänzende Perspektive.`);
    paras.push("Das Team kehrt zu seiner bisherigen Arbeitsweise zurück, die vor dem Eintritt der Person dominiert hat. Das kann kurzfristig Stabilität und Erleichterung bringen, weil die Anpassungsleistung wegfällt. Langfristig bedeutet es aber auch, dass die spezifische Ergänzung, die die Person eingebracht hat, wieder fehlt. Bei der nächsten Besetzung sollte geprüft werden, ob eine ähnliche Ergänzung erneut gesucht werden sollte.");
  } else if (c.matchCase === "TOP2_ONLY") {
