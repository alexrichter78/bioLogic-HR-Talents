...
function buildWarumText(c: Ctx): string {
  ...
  if (teamClass === "BALANCED" && personClass === "BALANCED") {
    ...
    if (matchCase === "TOP1_TOP2") {
      ...
      paras.push(`Die Hauptlogik unterscheidet sich deutlich: Das Team arbeitet primär über ${COMP_DOMAIN[teamPrimary]}, die Person über ${COMP_DOMAIN[personPrimary]}. Das bedeutet, dass Entscheidungen, Prioritäten und Kommunikation grundsätzlich anders angegangen werden. Im konkreten Arbeitsverhalten gibt es jedoch wichtige Überschneidungen: Beide Seiten nutzen ${COMP_SHORT[teamSecondary]} als ergänzende Arbeitsweise.`);
      paras.push("Das sorgt im Alltag für Anschlussfähigkeit und ermöglicht eine gemeinsame Arbeitsebene trotz unterschiedlicher Grundlogik. Die strukturelle Spannung bleibt aber bestehen und kann in Drucksituationen oder bei strategischen Entscheidungen stärker zutage treten. Die Integration erfordert deshalb ein klares Bewusstsein für diese Spannungsfelder und die Bereitschaft, sie offen anzusprechen.");
    } else {
...
function buildWirkungAlltagText(c: Ctx): string {
  ...
  if (matchCase === "TOP2_ONLY") {
    paras.push(`Im Alltag werden Unterschiede in der Grundhaltung spürbar. Das Team erwartet und lebt ${COMP_SHORT[teamPrimary]}, die Person bringt dagegen ${COMP_SHORT[personPrimary]} als primäre Arbeitslogik mit. Diese Unterschiede zeigen sich bei Entscheidungen, in der Kommunikation und bei der Priorisierung von Aufgaben. Das Team könnte den Eindruck gewinnen, dass die Person nicht so arbeitet, wie es erwartet wird.`);
    paras.push(`Im konkreten Tagesgeschäft gibt es jedoch Überschneidungen, die die Zusammenarbeit erleichtern: Beide Seiten setzen auf ${COMP_SHORT[c.teamSecondary]} als ergänzende Arbeitsweise. Diese gemeinsame Ebene ermöglicht es, im Alltag produktiv zusammenzuarbeiten, auch wenn die Grundlogik verschieden ist. Trotzdem braucht es klare Abstimmung und die bewusste Bereitschaft aller Beteiligten, die Unterschiede als Ergänzung zu sehen statt als Störung. Regelmässige Gespräche über gegenseitige Erwartungen und offene Kommunikation über Reibungspunkte sind hier besonders wichtig.`);
    return paras.join("\n\n");
  }
...
function buildChancenRisiken(c: Ctx): { chancen: V4Block[]; risiken: V4Block[]; chancenRisikenEinordnung: string } {
  ...
  if (c.matchCase === "TOP1_TOP2") {
    ...
    risiken.push({ title: "Führungsaufwand", text: "Die unterschiedliche Grundrichtung erfordert klare Erwartungen, regelmässige Abstimmung und eine aktive Führung, die beide Perspektiven integriert. Ohne diesen Steuerungsaufwand besteht die Gefahr, dass die Person schrittweise an den Rand gedrängt wird oder das Team sich gegen die neue Herangehensweise wehrt." });
...
function buildDruckText(c: Ctx): string {
  ...
  if (c.teamClass === "BALANCED" && c.personClass === "BALANCED") {
    ...
    paras.push(`Unter Druck verstärken sich die Unterschiede in der Grundlogik deutlich. Das Team zieht sich auf ${COMP_DOMAIN[c.teamPrimary]} zurück und reagiert ${COMP_ADJ[c.teamPrimary]}. Die Person verstärkt dagegen ${COMP_DOMAIN[c.personPrimary]} und reagiert ${COMP_ADJ[c.personPrimary]}. Diese gegensätzlichen Reaktionsmuster können in Stresssituationen zu offenen Konflikten führen.`);
    paras.push(`Die gemeinsame Arbeitsweise über ${COMP_SHORT[c.teamSecondary]} kann in solchen Momenten als Brücke dienen und den Kontakt zwischen beiden Seiten aufrechterhalten. Ob diese Brücke ausreicht, hängt von der Intensität des Drucks ab. Bei starkem Stress kann sie überlastet werden und die grundsätzlichen Unterschiede treten in den Vordergrund. Klare Eskalationswege, definierte Verantwortlichkeiten und frühzeitige Deeskalation sind hier besonders wichtig.`);
  } else {
...
function buildFuehrungshinweis(c: Ctx): V4Block[] | null {
  ...
  if (c.matchCase === "TOP1_TOP2") {
    ...
    hints.push({ title: "Erwartungen frühzeitig klären", text: "Die Führungskraft bringt eine Stärke mit, die das Team grundsätzlich kennt, aber auch Unterschiede in der konkreten Arbeitsweise, die Irritation auslösen können. Klären Sie frühzeitig und explizit, welche Arbeitsweise in welchen Situationen erwartet wird. Machen Sie transparent, wo die Führungskraft ihren eigenen Stil einbringen kann und wo Anpassung an die Teamkultur nötig ist. Diese Klärung verhindert, dass aus kleinen Irritationen grössere Konflikte werden." });
    ...
    hints.push({ title: "Stärke der Unterschiede kommunizieren", text: "Machen Sie dem Team klar, dass die Führungskraft bewusst gewählt wurde und dass ihre Andersartigkeit in bestimmten Bereichen ein Vorteil sein soll. Ohne diese Einordnung kann das Team die Unterschiede als Schwäche interpretieren statt als gewollte Ergänzung." });
  } else {
...
function buildEmpfehlungen(c: Ctx): V4Block[] {
  ...
  if (c.matchCase === "TOP1_TOP2") {
    ...
    emps.push({ title: "Erwartungen frühzeitig klären", text: `Die Person bringt mit ${COMP_DOMAIN[c.personPrimary]} eine grundlegend andere Denklogik mit als das Team. Klären Sie in den ersten Tagen frühzeitig und explizit, was in dieser Rolle erwartet wird, wo Spielraum für den eigenen Stil besteht und wo Anpassung an die Teamlogik nötig ist. Je klarer diese Grenzen definiert sind, desto einfacher wird die Integration, weil die Person weiss, wo sie sich frei bewegen kann und wo sie sich anpassen muss.` });
    emps.push({ title: "Ergänzung bewusst nutzen", text: `Die Stärke der Person in ${COMP_DOMAIN[c.personPrimary]} kann das Team gezielt bereichern und bestehende Lücken schliessen. Schaffen Sie bewusst Situationen und Aufgaben, in denen diese Stärke sichtbar wird und dem Team einen konkreten Mehrwert bringt. Das hilft dem Team, die Person nicht als Fremdkörper zu sehen, sondern als wertvolle Ergänzung. Geben Sie der Person explizit Aufgaben, die ihre besondere Stärke fordern.` });
    emps.push({ title: "Konflikte normalisieren", text: "Machen Sie dem Team von Anfang an klar, dass die Unterschiede gewollt sind und dass Reibung in dieser Konstellation ein normales und sogar gesundes Zeichen von Ergänzung sein kann. Schaffen Sie einen Rahmen, in dem Irritationen offen angesprochen werden können, ohne dass sie als persönlicher Angriff gewertet werden. Das senkt die Hemmschwelle für ehrliche Kommunikation und verhindert, dass Spannungen unterschwellig wachsen." });
  } else {
