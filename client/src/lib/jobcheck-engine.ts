...
function buildMatrix(role: RoleAnalysis, cand: CandidateInput): MatrixRow[] {
  ...
  rows.push({
    ...
    reasoning: dominanceStatus === "SUITABLE"
      ? "Die dominante Steuerungslogik ist kompatibel."
      : dominanceStatus === "CONDITIONAL"
        ? "Es gibt eine erkennbare Verschiebung in der Dominanzlogik. Passung möglich mit klarer Rahmensetzung."
        : "Die Kernlogik der Rolle wird durch die Kandidatenstruktur strukturell verschoben.",
  });

  const decisionStatus: FitStatus = (() => {
    if (rDom.top1.key === "impulsiv" && cDom.top1.key === "intuitiv" && r.impulsiv >= 50) return "NOT_SUITABLE";
    if (rDom.top1.key === "analytisch" && cDom.top1.key === "impulsiv" && r.analytisch >= 50) return "CONDITIONAL";
  ...
  rows.push({
    ...
    reasoning: decisionStatus === "SUITABLE"
      ? "Die Entscheidungslogik trifft die Kernanforderung der Rolle."
      : decisionStatus === "CONDITIONAL"
        ? "Die Entscheidungslogik weicht in Teilen ab. Erfordert klare Entscheidungsfristen und Eskalationsregeln."
        : "Die Rolle verlangt schnelle Durchsetzung. Das Profil priorisiert Stabilität/Abstimmung.",
  });
  ...
  rows.push({
    ...
    candidatePattern: c.analytisch >= 30 ? "Zahlen-/Strukturorientierung gut anschlussfähig" : "Kennzahlen eher als Orientierung, weniger als konsequenter Steuerungshebel",
    status: kpiStatus,
    reasoning: kpiStatus === "SUITABLE"
      ? "Die notwendige analytische Basis ist vorhanden."
      : "KPI-Disziplin ist umsetzbar, erfordert aber feste Routinen und konsequentes Nachhalten.",
  });
  ...
  if (role.leadership?.required) {
    ...
    rows.push({
      ...
      roleDemand: lp ? `${lp.impulsiv}% Impulsiv / ${lp.intuitiv}% Intuitiv / ${lp.analytisch}% Analytisch` : "Führung erforderlich",
      candidatePattern: `${c.impulsiv}% Impulsiv / ${c.intuitiv}% Intuitiv / ${c.analytisch}% Analytisch`,
      status: leadershipStatus,
      reasoning: leadershipStatus === "NOT_SUITABLE"
        ? "Die Rolle verlangt klare Ergebnisdominanz in der Führung. Das Profil ist strukturell zu wenig interventionsstark."
        : "Führung ist möglich, jedoch nur stabil mit klarer Zielarchitektur und Eskalationslogik.",
    });
  }
  ...
  rows.push({
    ...
    reasoning: conflictStatus === "NOT_SUITABLE"
      ? "Unter Zielabweichung wird wahrscheinlich zu viel moderiert statt konsequent entschieden."
      : "Konfliktfähigkeit ist grundsätzlich vorhanden, muss aber in Performance-Situationen konsequent aktiviert werden.",
  });
  ...
  rows.push({
    ...
    candidatePattern: c.impulsiv >= 45 ? "Tempo-/Abschlussorientierung gut" : "Tempo eher zugunsten Stabilität/Abstimmung reduziert",
    status: competitionStatus,
    reasoning: competitionStatus === "NOT_SUITABLE"
      ? "Im Hochdruckumfeld ist impulsive Interventionslogik zentral. Das Profil verschiebt die Dynamik."
      : "Die Dynamik ist steuerbar, wenn Prioritäten und Zielhärte klar etabliert sind.",
  });

  const cultureStatus: FitStatus = (() => {
    if (r.impulsiv >= 60 && c.intuitiv >= 50) return "CONDITIONAL";
    if (c.intuitiv >= 45) return "SUITABLE";
    return "CONDITIONAL";
  })();
  ...
  rows.push({
    ...
    reasoning: cultureStatus === "SUITABLE"
      ? "Das Profil kann Teamstabilität und Motivation stärken."
      : "Risiko der Kulturverschiebung. Nur tragfähig, wenn Leistungsarchitektur klar bleibt.",
  });
  ...
  if (tags.sales_cycle === "lang") {
    rows.push({
      ...
      candidatePattern: c.analytisch >= 30 ? "Struktur und Planung anschlussfähig" : "Mehr situativ als systematisch",
      status: "CONDITIONAL",
      reasoning: "Bei langen Zyklen wird Struktur wichtiger. Entscheidend ist systematische Arbeitsweise.",
    });
  }
  ...
  if (tags.regulation === "hoch") {
    rows.push({
      ...
      status: "CONDITIONAL",
      reasoning: "In regulierten Umfeldern muss Präzision stabil sein. Erfordert klare Standards und Kontrollen.",
    });
  }
  ...
  if (tags.customer_type) {
    rows.push({
      ...
      roleDemand: tags.customer_type === "B2C" ? "Hohe Frequenz, klare Abschlussorientierung" : tags.customer_type === "B2B" ? "Beziehungsstabilität + wirtschaftliche Steuerung" : "Situativ",
      candidatePattern: c.intuitiv >= 45 ? "Beziehungsaufbau stark" : "Eher transaktional",
      status: "CONDITIONAL",
      reasoning: "Kundenorientierung grundsätzlich vorhanden; entscheidend ist die Balance aus Beziehung und Abschlusslogik.",
    });
  }
