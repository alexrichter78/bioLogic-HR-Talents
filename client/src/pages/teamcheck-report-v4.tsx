...
export default function TeamCheckReportV4() {
  ...
  const axisLabel = (v: string) => v === "hoch" ? ui.teamreport.high : v === "mittel" ? ui.teamreport.medium : v === "gering" ? ui.teamreport.low : ui.teamreport.notAssessable;
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: isMobile ? "64px 12px 80px" : "80px 20px 48px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }} className="no-print">
          <button onClick={() => navigate("/team-report")} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "#1A5DAB", fontWeight: 600, fontSize: 14, padding: 0 }} data-testid="button-back-v4">
            <ArrowLeft size={16} /> {ui.teamreport.backToTeamCheck}
          </button>
        </div>
        ...
        <div ref={reportRef} data-testid="v4-report-wrapper">
          <div style={{ position: "relative", background: "#F8F9FB", borderRadius: 20, overflow: "hidden", boxShadow: "0 4px 40px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.03)" }}>
            ...
            <div data-pdf-block className="report-header report-header--auto" data-testid="v4-header">
              ...
              <div className="report-kicker">{ui.teamreport.kicker}</div>
              <h1 className="report-title report-title--flow">{ui.teamreport.title}</h1>
              <div className="report-subtitle report-subtitle--flow">{result.roleTitle || ui.teamreport.teamSimulation}</div>
...
[data-pill]{white-space:nowrap!important}
                  ...
                  title={ui.teamreport.printTitle}
                >
                  <Printer style={{ width: 15, height: 15 }} />
                  <span>{ui.teamreport.printBtn}</span>
                </button>
              </div>
            </div>
            ...
            <div style={{ padding: isMobile ? "20px 16px 0" : "28px 32px 0" }}>
              ...
              <SectionHead num={1} title={ui.teamreport.overallAssessment} id="gesamtbewertung" />
              ...
              <div style={{ margin: "0 0 24px" }} data-testid="v4-hero-bewertung">
                ...
                <div data-pdf-block style={{ display: "flex", gap: 16, marginTop: 16, breakInside: "avoid" }} data-testid="v4-two-axis">
                  <div style={{ flex: 1, padding: "12px 16px", borderRadius: 10, background: `${bCol}08`, border: `1px solid ${bCol}25` }} data-testid="v4-gesamt-card">
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>{ui.teamreport.overallLabel}</div>
                  ...
                  <div style={{ flex: 1, padding: "12px 16px", borderRadius: 10, background: `${axisColor(result.passungZumTeam)}08`, border: `1px solid ${axisColor(result.passungZumTeam)}25` }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>{ui.teamreport.teamFit}</div>
                ...
                {(() => {
                  const COMP_LABEL: Record<string, string> = {
                    impulsiv: ui.general.labelImpulsivArea,
                    intuitiv: ui.general.labelIntitivArea,
                    analytisch: ui.general.labelAnalytischArea,
                  };
                  ...
                  return (
                    <div data-pdf-block style={{ marginTop: 16, padding: "20px 24px", borderRadius: 12, background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)", breakInside: "avoid" }} data-testid="v4-kurzuebersicht-dominanz">
                      <p style={{ fontSize: 14, fontWeight: 700, color: "#1D1D1F", margin: "0 0 16px", textAlign: "center" }}>{ui.teamreport.overview}</p>
                ...
                <div data-pdf-block style={{ display: "flex", gap: 16, marginTop: 16, breakInside: "avoid" }} data-testid="v4-kurzueberblick">
                  <div style={{ flex: 1, padding: "12px 16px", borderRadius: 10, background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)" }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>{ui.teamreport.keyStrength}</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "#1D1D1F", lineHeight: 1.5 }} data-testid="v4-hauptstaerke">{result.hauptstaerke}</div>
                  </div>
                  <div style={{ flex: 1, padding: "12px 16px", borderRadius: 10, background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)" }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>{ui.teamreport.keyRisk}</div>
            ...
            <div style={{ padding: isMobile ? "0 16px 32px" : "0 32px 48px" }}>
              ...
              {(() => {
                const COMP_LABEL_FULL: Record<string, string> = {
                  impulsiv: ui.general.labelImpulsiv,
                  intuitiv: ui.general.labelIntuitiv,
                  analytisch: ui.general.labelAnalytisch,
                };
                const COMP_LABEL_AREA: Record<string, string> = {
                  impulsiv: ui.general.labelImpulsivArea,
                  intuitiv: ui.general.labelIntitivArea,
                  analytisch: ui.general.labelAnalytischArea,
                };
                    ...
                    <div data-pdf-block style={{ marginTop: 20, padding: "18px 20px", borderRadius: 12, background: "#F8F9FA", border: "1px solid rgba(0,0,0,0.06)" }}>
                      ...
                      <div style={{ display: "flex", gap: 12, flexDirection: isMobile ? "column" : "row" }}>
                        {([
                          ...
                          { key: "intuitiv" as ComponentKey, label: ui.general.labelIntuitiv, color: COMP_HEX.intuitiv, text: ui.teamreport.intuitivDesc },
                        ]).map(kb => (
              ...
              <div data-pdf-block style={sectionStyle} data-testid="v4-section-warum">
                <SectionHead num={3} title={ui.teamreport.whyResult} id="warum" />
              ...
              <div data-pdf-block style={sectionStyle} data-testid="v4-section-wirkung">
                <SectionHead num={4} title={ui.teamreport.dailyImpact} id="wirkung" />
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 12 : 16, marginBottom: 24 }}>
                  <div style={{ padding: "20px", borderRadius: 12, background: "rgba(52,199,89,0.04)", border: "1px solid rgba(52,199,89,0.15)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                      ...
                      <span style={{ fontSize: 15, fontWeight: 700, color: "#1B7A3D" }}>{ui.teamreport.chancen}</span>
                    </div>
                  ...
                  <div style={{ padding: "20px", borderRadius: 12, background: "rgba(255,59,48,0.03)", border: "1px solid rgba(255,59,48,0.12)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                      ...
                      <span style={{ fontSize: 15, fontWeight: 700, color: "#C41E3A" }}>{ui.teamreport.risiken}</span>
                    </div>
              ...
              <div data-pdf-block style={sectionStyle} data-testid="v4-section-druck">
                <SectionHead num={6} title={ui.teamreport.pressure} id="druck" />
              ...
              {result.fuehrungshinweis && (
                <div data-pdf-block style={sectionStyle} data-testid="v4-section-fuehrung">
                  <SectionHead num={7} title={ui.teamreport.leadershipNote} id="fuehrung" />
              ...
              {(() => {
                ...
                return (
                  <div style={sectionStyle} data-testid="v4-section-integrationsplan">
                    <SectionHead num={planNum} title={ui.teamreport.integrationPlan} id="integrationsplan" />
                          ...
                          <div key={phase.num} data-testid={`v4-integration-phase-${phase.num}`} style={{ borderRadius: 14, overflow: "hidden", border: `1px solid ${phaseCol}20` }}>
                            ...
                            <div style={{ padding: "16px 20px" }}>
                              <p style={{ fontSize: 14, fontWeight: 600, color: "#1D1D1F", margin: "0 0 14px", lineHeight: 1.7 }}>
                                <span style={{ fontWeight: 700 }}>{ui.teamreport.phaseGoal}{" "}</span>{phase.ziel}
                              </p>

                              <div style={{ marginBottom: 20 }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: "#1D1D1F", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 8px" }}>{ui.teamreport.whatMatters}</p>
                              ...
                              <div data-pdf-block style={{ marginBottom: 20 }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: "#1D1D1F", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 8px" }}>{ui.teamreport.inPractice}</p>
                              ...
                              <div data-pdf-block style={{ marginBottom: 20 }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: "#34C759", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 8px" }}>{ui.teamreport.signsWorking}</p>
                              ...
                              <div data-pdf-block style={{ padding: "14px 16px", borderRadius: 10, background: "rgba(0,0,0,0.02)", borderLeft: "3px solid #8E8E93", marginBottom: 16 }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 6px" }}>{ui.teamreport.leadershipNoteLabel}</p>
                              ...
                              <div data-pdf-block style={{ padding: "14px 16px", borderRadius: 10, background: `${phaseCol}06`, borderLeft: `4px solid ${phaseCol}` }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: phaseCol, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 6px" }}>{ui.teamreport.focusAreas}</p>
                    ...
                    <div data-pdf-block style={{ marginTop: 28, padding: "18px 20px", borderRadius: 12, background: "#FFF5F5", border: "1px solid rgba(196,30,58,0.12)" }} data-testid="v4-integration-warnsignale">
                      <p style={{ fontSize: 11, fontWeight: 700, color: "#C41E3A", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 10px" }}>{ui.teamreport.warningSignals}</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    <div data-pdf-block style={{ marginTop: 16, padding: "18px 20px", borderRadius: 12, background: "rgba(0,113,227,0.03)", border: "1px solid rgba(0,113,227,0.12)" }} data-testid="v4-integration-leitfragen">
                      <p style={{ fontSize: 11, fontWeight: 700, color: "#0071E3", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 10px" }}>{ui.teamreport.guideQuestions}</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    <div data-pdf-block style={{ marginTop: 16, padding: "16px 18px", borderRadius: 10, background: "rgba(0,0,0,0.02)", borderLeft: "3px solid #8E8E93" }} data-testid="v4-integration-verantwortung">
                      <p style={{ fontSize: 11, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 6px" }}>{ui.teamreport.important}</p>
              ...
              {(() => {
                ...
                return (
                  <div data-pdf-block style={sectionStyle} data-testid="v4-section-risikoprognose">
                    <SectionHead num={riskNum} title={ui.teamreport.riskForecast} id="risikoprognose" />
              ...
              {(() => {
                ...
                return (
                  <div data-pdf-block style={sectionStyle} data-testid="v4-section-team-ohne-person">
                    <SectionHead num={ohneNum} title={ui.teamreport.withoutPerson} id="team-ohne-person" />
              ...
              {(() => {
                ...
                return (
                  <div data-pdf-block style={sectionStyle} data-testid="v4-section-empfehlungen">
                    <SectionHead num={empNum} title={ui.teamreport.keyRecommendations} id="empfehlungen" />
              ...
              {(() => {
                ...
                return (
                  <div data-pdf-block style={{ marginBottom: 36 }} data-testid="v4-section-schlussfazit">
                    <SectionHead num={fazitNum} title={ui.teamreport.closing} id="schlussfazit" />
              ...
              <div style={{ padding: "24px 0", borderTop: "1px solid rgba(0,0,0,0.08)" }} data-testid="v4-footer">
                <p style={{ fontSize: 12, color: "#B0B0B5", margin: 0, textAlign: "center", lineHeight: 1.6 }}>
                  © {new Date().getFullYear()} bioLogic Talent Navigator · {ui.teamreport.fitAnalysis} · {ui.teamreport.generatedOn} {today}
                </p>
              </div>
