      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center" }}>
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#0071E3" }}>{ui.kurs.modules}</span>
          </div>
          <div style={{
            display: "flex", alignItems: "center", gap: 8,
            padding: "7px 16px", borderRadius: 10,
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#0071E3" }}>{ui.kurs.video}</span>
          </div>
        </div>
      </div>
      ...
      <div className="page-gradient-bg">
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", paddingTop: isMobile ? 130 : 160, padding: isMobile ? "130px 16px 40px" : "160px 24px 40px", textAlign: "center" }}>
          <div style={{
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1D1D1F", margin: "0 0 8px" }} data-testid="text-no-access">{ui.kurs.noAccess}</h2>
            <p style={{ fontSize: 14, color: "#48484A", maxWidth: 400, margin: "0 auto 24px" }}>
              {ui.kurs.noAccessDesc}
            </p>
            <button
              onClick={() => setLocation("/")}
      ...
      <div className="page-gradient-bg">
        ...
        <div className="mx-auto" style={{ maxWidth: 1100, paddingTop: isMobile ? 110 : 135, paddingBottom: isMobile ? 100 : 40, paddingLeft: isMobile ? 8 : 24, paddingRight: isMobile ? 8 : 24 }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 17, fontWeight: 700, color: "#1D1D1F", margin: "0 0 8px" }}>{ui.kurs.comingSoon}</p>
            <p style={{ fontSize: 14, color: "#6E6E73", margin: 0, maxWidth: 500, marginLeft: "auto", marginRight: "auto" }}>
              {ui.kurs.comingSoonDesc}
            </p>
          </div>
        </div>
      </div>
        ...
        <div style={{
          ...
          {submitted ? (
            <div style={{ padding: isMobile ? "32px 20px" : "40px 32px", textAlign: "center" }} data-testid="card-success">
              ...
              <p style={{ fontSize: 18, fontWeight: 700, color: "#1D1D1F", margin: "0 0 8px" }}>{ui.kurs.successTitle}</p>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div>
                {!isMobile && (
                  <div style={{
                    ...
                    <span />
                    <span style={{ ...labelStyle, paddingLeft: 14 }}>{ui.kurs.firstName}</span>
                    <span style={{ ...labelStyle, paddingLeft: 14 }}>{ui.kurs.lastName}</span>
                    <span style={{ ...labelStyle, paddingLeft: 14 }}>{ui.kurs.email}</span>
                    <span />
                  </div>
                )}
                ...
                {participants.map((p, idx) => (
                  ...
                  >
                    ...
                    {isMobile && <label style={labelStyle}>{ui.kurs.firstName}</label>}
                    <input type="text" value={p.firstName} onChange={e => updateParticipant(p.id, "firstName", e.target.value)}
                      placeholder={ui.kurs.firstName} style={inputStyle} required
                    ...
                    {isMobile && <label style={labelStyle}>{ui.kurs.lastName}</label>}
                    <input type="text" value={p.lastName} onChange={e => updateParticipant(p.id, "lastName", e.target.value)}
                      placeholder={ui.kurs.lastName} style={inputStyle} required
                      onInvalid={e => { const el = e.target as HTMLInputElement; el.setCustomValidity(ui.login.emailRequired); }}
                      onInput={e => (e.target as HTMLInputElement).setCustomValidity("")}
                      data-testid={`input-lastname-${p.id}`} />

                    {isMobile && <label style={labelStyle}>{ui.kurs.email}</label>}
                    <input type="email" value={p.email} onChange={e => updateParticipant(p.id, "email", e.target.value)}
                      placeholder={ui.kurs.email} style={inputStyle} required
              ...
              <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 12, padding: isMobile ? "20px 16px" : "24px 32px", justifyContent: "center", alignItems: "center" }}>
                ...
                >
                  ...
                  {submitting ? ui.kurs.unlocking : ui.kurs.unlockAccess}
                </button>
              </div>
            </form>
          )}
