...
- Kurz, konkret, maximal 3 Sätze`;
  ...
  const BANNED_PHRASES = [
    "darüber hinaus", "insbesondere", "im Kontext von", "maßgeblich",
    "ganzheitlich", "vielmehr", "nicht nur", "sondern auch",
    "auf Basis von", "an dieser Stelle", "entsprechend", "grundsätzlich",
    "im Rahmen",
  ];

  function antiKiFilter(text: string): string {
    let result = text;
    for (const phrase of BANNED_PHRASES) {
      const regex = new RegExp(phrase, "gi");
      result = result.replace(regex, "");
    }
    result = result.replace(/\s{2,}/g, " ").replace(/ ([.,;])/g, "$1");
    return result.trim();
  }

  const GLOBAL_STYLE = `Du schreibst einen professionellen HR-Diagnosebericht für Recruiting- und Besetzungsentscheidungen.
Zielgruppe sind Personaler, Headhunter, Führungskräfte und Geschäftsführer.
Der Text soll klingen wie ein klarer Managementbericht, nicht wie ein psychologischer Fachtext und nicht wie ein KI-Text.
Schreibe in natürlicher, ruhiger, professioneller Sprache.

Regeln:
- kurze bis mittellange Sätze
- keine Marketing-Sprache
- keine psychologischen Fachbegriffe ohne Erklärung
- keine KI-typischen Formulierungen wie: "darüber hinaus", "insbesondere", "im Kontext von", "maßgeblich", "ganzheitlich", "vielmehr", "nicht nur ..., sondern auch ..."
- keine Floskeln
- keine Aufzählungen mitten im Satz
- jeder Absatz maximal 3 Sätze
- kein werblicher Ton
- keine übertriebene Dramatik
- klar, konkret, verständlich
- KEINE Gedankenstriche (–)

Der Bericht beschreibt nicht die Person allgemein, sondern die Wirkung eines Kandidaten in Bezug auf eine konkrete Rolle.
Die Begriffe impulsiv, intuitiv und analytisch dürfen nur in Überschriften oder Legenden stehen.
Im Fließtext nutze stattdessen:
- impulsiv = Umsetzung, Tempo, Entscheidung, Handlungsfokus
- intuitiv = Zusammenarbeit, Gespür für Menschen, Kommunikation, Kontext
- analytisch = Struktur, Planung, Sorgfalt, Prüftiefe

Antworte NUR mit dem Fließtext. Keine Überschriften, keine Markdown-Formatierung, keine Aufzählungszeichen.`;

  type SollIstSectionId = "management_summary" | "target_profile" | "candidate_profile" | "dominance_shift" | "impact_analysis" | "stress_logic" | "risk_timeline" | "development" | "actions" | "final_assessment";

  function componentLabel(k: string) {
    if (k === "impulsiv") return "Umsetzung/Tempo";
    if (k === "intuitiv") return "Zusammenarbeit/Kommunikation";
    return "Struktur/Planung";
  }

  function dominantComponent(t: { impulsiv: number; intuitiv: number; analytisch: number }) {
  ...
  function buildSectionPrompt(
    sectionId: SollIstSectionId,
    role: any,
    candidate: any
  ): string {
    const rp = role.role_profile;
    const cp = candidate.candidate_profile;
    const roleDom = dominantComponent(rp);
    const candDom = dominantComponent(cp);
    const mainGap = Math.abs(rp.impulsiv - cp.impulsiv) + Math.abs(rp.intuitiv - cp.intuitiv) + Math.abs(rp.analytisch - cp.analytisch);
    const gapLevel = mainGap > 40 ? "hoch" : mainGap > 20 ? "mittel" : "gering";
    const overallRating = mainGap > 40 ? "nicht geeignet" : mainGap > 20 ? "bedingt geeignet" : "geeignet";
    const controlIntensity = mainGap > 40 ? "hoch" : mainGap > 20 ? "mittel" : "gering";

    switch (sectionId) {
      case "management_summary":
        return `Erstelle den Abschnitt "Management Summary".
Fasse das Ergebnis in 3 kurzen Absätzen zusammen.
1. Wie ist die Gesamteinstufung?
2. Warum ist der Kandidat ${overallRating}?
3. Welche Arbeitslogik verschiebt sich?

Rolle: ${role.job_title}
Gesamteinstufung: ${overallRating}
Steuerungsintensität: ${controlIntensity}
Soll-Dominanz: ${componentLabel(roleDom)}
Ist-Dominanz: ${componentLabel(candDom)}
Soll-Werte: Imp ${rp.impulsiv}, Int ${rp.intuitiv}, Ana ${rp.analytisch}
Ist-Werte: Imp ${cp.impulsiv}, Int ${cp.intuitiv}, Ana ${cp.analytisch}
Kandidat: ${candidate.candidate_name || "Kandidat"}

Maximal 3 Absätze. Sachlich und klar. Direkt entscheidungsrelevant.`;

      case "target_profile":
        return `Erstelle den Abschnitt "Rollenprofil (Soll)".
Erkläre in 1 bis 2 Absätzen, welche Arbeitslogik die Rolle verlangt und warum.

Rolle: ${role.job_title}
Soll-Werte: Imp ${rp.impulsiv}, Int ${rp.intuitiv}, Ana ${rp.analytisch}
Dominanz: ${componentLabel(roleDom)}
Erfolgsfokus: ${(role.success_metrics || []).join(", ") || "keine Angabe"}
Arbeitslogik: ${role.job_family || "keine Angabe"}
Führungsrolle: ${role.leadership?.type || "Keine"}

Beschreibe die Rolle so, dass ein Personaler versteht, welche Art von Person diese Struktur tragen kann.`;

      case "candidate_profile":
        return `Erstelle den Abschnitt "Kandidatenprofil (Ist)".
Beschreibe in 1 bis 2 Absätzen die Arbeits- und Verhaltenslogik des Kandidaten.

Kandidat: ${candidate.candidate_name || "Kandidat"}
Werte: Imp ${cp.impulsiv}, Int ${cp.intuitiv}, Ana ${cp.analytisch}
Dominanz: ${componentLabel(candDom)}

Beschreibe beobachtbares Verhalten im Arbeitsalltag. Nicht psychologisch.`;

      case "dominance_shift":
        return `Erstelle den Abschnitt "Dominanz-Verschiebung".
Erkläre, wie sich die Wirkung der Rolle verändert, wenn Soll und Ist nicht übereinstimmen.

Soll-Dominanz: ${componentLabel(roleDom)}
Ist-Dominanz: ${componentLabel(candDom)}
Rolle: ${role.job_title}
Soll: Imp ${rp.impulsiv}, Int ${rp.intuitiv}, Ana ${rp.analytisch}
Ist: Imp ${cp.impulsiv}, Int ${cp.intuitiv}, Ana ${cp.analytisch}

Schreibe 2 kurze Absätze:
1. Was die Rolle braucht
2. Wie das Profil des Kandidaten die Wirkung verschiebt`;

      case "impact_analysis":
        return `Erstelle den Abschnitt "Strukturelle Wirkungsanalyse".
Beschreibe die Auswirkungen der Soll-Ist-Abweichung in diesen Bereichen:
- Entscheidungslogik
- Arbeitssteuerung und Dokumentation
- Führungswirkung
- Konfliktfähigkeit
- Kulturwirkung
...
Pro Bereich 2 bis 3 Sätze. Immer erklären: Was die Rolle braucht, was der Kandidat stattdessen mitbringt, welches Risiko daraus entsteht.`;

      case "stress_logic":
        return `Erstelle den Abschnitt "Wirkung unter Druck und Stress".
2 Teile:

1. Verhalten unter Druck:
- Welche Arbeitslogik verstärkt sich unter Belastung?
- Welche Rollenanforderungen geraten in den Hintergrund?

2. Verhalten bei starkem Stress:
- Wie verändert sich das Verhalten bei dauerhaft hoher Belastung?
- Welche Risiken entstehen für Qualität, Tempo, Struktur oder Zusammenarbeit?
...
Beobachtbares Verhalten beschreiben, nicht theoretisch.`;

      case "risk_timeline":
        return `Erstelle den Abschnitt "Risikoprognose".
Beschreibe Risiken in drei Zeiträumen:
- kurzfristig (0-3 Monate)
- mittelfristig (3-12 Monate)
- langfristig (12+ Monate)

Rolle: ${role.job_title}
Abweichungsniveau: ${gapLevel}
Soll-Dominanz: ${componentLabel(roleDom)}
Ist-Dominanz: ${componentLabel(candDom)}

Pro Zeitraum 2 Sätze. Erst Wirkung auf Arbeitsweise, dann auf Ergebnisse. Keine Katastrophensprache.`;

      case "development":
        return `Erstelle den Abschnitt "Entwicklungsprognose".
Beschreibe realistisch, wie wahrscheinlich eine Anpassung des Kandidaten an die Rolle ist.
...
Nicht absolut formulieren. Realistisch und professionell. 1-2 Absätze.`;

      case "actions":
        return `Erstelle den Abschnitt "Handlungsempfehlung".
Beschreibe Steuerungsmaßnahmen, wenn die Besetzung dennoch erfolgt.
...
Klare, konkrete Maßnahmen. Direkt umsetzbar. Geeignet für Führungskräfte. 2-3 Absätze.`;

      case "final_assessment":
        return `Erstelle den Abschnitt "Gesamtbewertung".
Kurzer Abschluss mit: Grundpassung, Steuerungsbedarf, zentrale Begründung, klare Empfehlung.

Rolle: ${role.job_title}
Grundpassung: ${overallRating}
Steuerungsbedarf: ${controlIntensity}
Soll-Dominanz: ${componentLabel(roleDom)}
Ist-Dominanz: ${componentLabel(candDom)}
Kandidat: ${candidate.candidate_name || "Kandidat"}

Maximal 4 Sätze. Klar, sachlich, entscheidungsorientiert.`;
  ...
  app.post("/api/generate-soll-ist-section", async (req, res) => {
    try {
      const { role, candidate, sectionId } = req.body;
      if (!role || !candidate || !sectionId) {
        return res.status(400).json({ error: "Fehlende Parameter" });
      }

      const sectionPrompt = buildSectionPrompt(sectionId as SollIstSectionId, role, candidate);
      if (!sectionPrompt) {
        return res.status(400).json({ error: "Unbekannter Abschnitt" });
      }
      ...
      const response = await openai.chat.completions.create({
        ...
        max_tokens: 800,
        messages: [
          { role: "system", content: GLOBAL_STYLE },
          { role: "user", content: sectionPrompt },
        ],
      });

      let text = response.choices[0]?.message?.content?.trim() || "";
      text = antiKiFilter(text);

      res.json({ text });
    } catch (error) {
      console.error("Error generating Soll-Ist section:", error);
      res.status(500).json({ error: "Fehler bei der Generierung" });
    }
  });
