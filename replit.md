...
-   **JobCheck Baseline-Korrektur (Apr 2026)**: Bereichswerte für Tätigkeiten / Humankompetenzen / Rahmenbedingungen / Führungskompetenzen werden in `client/src/lib/jobcheck-engine.ts` (`calcBG`, `calcRahmen`) mit der 33,33 %-Baseline gemischt (Mischfaktor `JOBCHECK_BASELINE_MIX = 0.5`, exportiert). So liegen Soll-Werte im realistischen Korridor (max ~50 %) statt im theoretischen 0–100 %. Die Match-Toleranzen in `computeCoreFit()` sind gemeinsam mit der Stauchung skaliert: `GOOD_TOL = 3`, `COND_TOL = 7` (vorher 5 / 10). Dominanz-Reihenfolge der Bereiche bleibt mathematisch erhalten. `EQ_TOL = 5` für Strukturvarianten unverändert (gleiche Logik wie in den anderen Engines).
