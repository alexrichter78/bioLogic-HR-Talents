...
-   **Live Translation System**: DB-backed translation table (`translations`) with 333 entries across all 4 languages (sections: Navigation, Login, Allgemein, Kurs, TeamCheck Eingabe, TeamCheck Bericht, MatchCheck, Rollen-DNA, RollenProfil, TeamDynamik, Analyse). Admin page at `/ubersetzung` provides inline cell editing — click any DE/EN/FR/IT text, edit, press Enter → saved instantly via PATCH `/api/translations/:key`. `TranslationProvider` wraps the app, seeds DB on first admin login (via `all-translations.ts`), and `useUI()` deep-merges DB overrides over hardcoded `ui-texts.ts` values. Changes reflect across the entire app immediately. New `POST /api/translations/sync` endpoint inserts missing keys without overwriting admin edits — called automatically when admin opens `/ubersetzung`. Pages fully wired to `useUI()`: login, kurs, analyse, rollenprofil (PDF error), rollen-dna (12 key UI strings), soll-ist-bericht (10 key UI strings), teamdynamik (3 key strings). Pages with `useUI()` hook added (partial): teamcheck, team-report.
