...
-   **PDF Export (Passungsbericht)**: Uses a separate flat PDF layout component (`client/src/components/pdf-flat-report.tsx`) rendered off-screen via `createRoot`. This avoids html2canvas issues with SVG icons, absolute positioning, and complex CSS. The browser report shows the rich visual layout; the PDF gets a clean, typographic layout with tables, text bullets, and separator lines. Cleanup is exception-safe (always unmounts React root and removes container in `finally`).
...
-   **TeamCheck Report V3 PDF Export**: `client/src/lib/teamcheck-pdf-builder.ts` generates a styled PDF matching the MatchCheck PDF design — dark anthrazit header with logo/badges, 12 colored section bars with number chips, bar charts for profile comparison, risk timeline, and all TeamCheck sections. Triggered by PDF button on the report page.
-   **TeamCheck Report V3 (teamcheck-report-v3.tsx)**: Combined team system report at `/teamcheck-report-v3`, launched via "Testbericht 3" button from TestTeamReport. Uses `computeTeamCheckV3()` from `teamcheck-v3-engine.ts`, which merges data from both `team-report-engine.ts` (V1) and `teamcheck-v2-engine.ts` (V2). Data passed via sessionStorage (`teamcheckV3Input`). 12-section structure: 1-Systemstatus (management overview), 2-Managementfazit (3-4 sentences from V1 managementSummary + V2 passung), 3-Warum dieses Ergebnis (merged V2 reasons + V1 entscheidungsfaktoren), 4-Systemwirkung (V2 systemwirkungText + profile descriptions), 5-Strukturvergleich (bar charts + deviation), 6-Team-Spannungsanalyse (V2 tension items), 7-Auswirkungen im Arbeitsalltag (merged V2 impacts + V1 impactAreas + Kulturwirkung), 8-Verhalten unter Druck (V2 stress), 9-Risikoentwicklung über Zeit (V1 riskTimeline with fallback), 10-Chancen (V2), 11-Risiken (V2), 12-Handlungsempfehlung (V2 advice).
