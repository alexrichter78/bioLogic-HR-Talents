...
-   **TeamCheck Report V3 (teamcheck-report-v3.tsx)**: Combined team system report at `/teamcheck-report-v3`, launched via "Testbericht 3" button from TestTeamReport. Uses `computeTeamCheckV3()` from `teamcheck-v3-engine.ts`. **3-dimensional text logic**: All text generation now evaluates three axes simultaneously — Person↔Team, Person↔Funktionsziel, Team↔Funktionsziel — with decision rules producing labels: stabil passend, spannungsreiche Ergänzung, strategisch passende Korrektur, gezielte Ergänzung, kritische Abweichung, strukturelle Fehlpassung. **Executive Decision Page** (dark header): Systemstatus (5 metrics when goal set: Gesamtpassung, Systemwirkung, Steuerungsaufwand, Integrationsrisiko, Strategische Wirkung), Systemüberblick (Team/Person profiles + Abweichung + Funktionsziel + Team–Funktionsziel gap + Person–Funktionsziel gap), Strategische Einordnung, Strukturkonstellation, Managementkurzfazit, Integrationsfaktor. Then 12 numbered sections: 2-Warum dieses Ergebnis, 3-Systemwirkung (with dual soziale/funktionale Systemwirkung text), 4-Strukturvergleich, 5-Team-Spannungsanalyse (with goal-dimension enrichment), 6-Auswirkungen im Arbeitsalltag, 7-Auswirkungen auf Leistung und Ergebnisse, 8-Verhalten unter Druck, 9-Risikoentwicklung, 10-Chancen, 11-Risiken, 12-Handlungsempfehlung (with "Strategische Wirkung offen einordnen" first recommendation when applicable), 13-Alternativwirkung (includes functional gap persistence). GoalContext computed via GOAL_TRIADS ideal profiles; teamGoalAbweichung and personGoalAbweichung calculated as distances to goal triad.
