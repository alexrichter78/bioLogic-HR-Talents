...
-   **TeamCheck Report V4 / Integrationsanalyse (teamcheck-report-v4.tsx)**: 8-section integration analysis at `/teamcheck-report-v4`, launched via "Test4" button from TestTeamReport. Uses `computeTeamCheckV4()` from `teamcheck-v4-engine.ts`, which wraps V3 engine and restructures outputs. Two role variants: Führungskraft vs Teammitglied (controlled by `roleTypeForCard` state). Data passed via sessionStorage (`teamcheckV4Input`). Updated labels: Stimmig/Bedingt stimmig/Kritisch (instead of Passend), Systemwirkung: Stabilisiert/Ergänzt/Verändert/Erzeugt Spannung, Risikoniveau: niedrig/erhöht/hoch. 8 sections: 1-Management Summary (Kurzfazit + Erste Empfehlung), 2-Warum dieses Ergebnis (Ausgangslage/Gemeinsame Stärke/Kritische Abweichung/Bedeutung Funktionsziel), 3-Wirkung im System (variant: Teammitglied=4 team impact blocks, Führungskraft=Führungswirkung/Anschluss/Spannungsfelder/Führungsbedarf), 4-Chancen und Risiken (side-by-side), 5-Was ohne Besetzung bleibt, 6-Auswirkungen im Alltag (compact boxes), 7-Leistung und Ergebnisse (kurzfristig/mittelfristig/qualität/geschwindigkeit), 8-Verhalten unter Druck + Handlungsempfehlungen. Plus Strukturvergleich with bar charts.
-   **TeamCheck Report V3 (teamcheck-report-v3.tsx)**: Combined team system report at `/teamcheck-report-v3`, launched via "Testbericht 3" button from TestTeamReport. Uses `computeTeamCheckV3()` from `teamcheck-v3-engine.ts`. **3-dimensional text logic**: All text generation evaluates three axes simultaneously — Person↔Team, Person↔Funktionsziel, Team↔Funktionsziel — with decision rules producing labels: stabil passend, spannungsreiche Ergänzung, strategisch passende Korrektur, gezielte Ergänzung, begrenzt anschlussfähig, kritische Abweichung, strukturelle Fehlpassung. **Critical divergence flag `personDivergesFromGoal`** (teamAligned=true AND personAligned=false): When person deviates from team AND does NOT support the functional goal, ALL text consistently reflects critical assessment — Systemwirkung becomes "Kritische Spannung" or "Spannungsreiche Abweichung", chances are modest (punktuell), risks highlight Zielverschiebung, advice leads with "Abweichende Wirkung klar einordnen", alternativwirkung notes team stays closer to goal without person. **Executive Decision Page** (dark header): Systemstatus (5 metrics when goal set: Gesamtpassung, Systemwirkung, Steuerungsaufwand, Integrationsrisiko, Strategische Wirkung), Systemüberblick (Team/Person profiles + Abweichung + Funktionsziel + Team–Funktionsziel gap + Person–Funktionsziel gap), Strategische Einordnung, Strukturkonstellation, Managementkurzfazit, Integrationsfaktor. Then 12 numbered sections: 2-Warum dieses Ergebnis, 3-Systemwirkung (with dual soziale/funktionale Systemwirkung text), 4-Strukturvergleich, 5-Team-Spannungsanalyse (with goal-dimension enrichment + non-goal dimension caution for diverging cases), 6-Auswirkungen im Arbeitsalltag, 7-Auswirkungen auf Leistung und Ergebnisse (realistic when diverging), 8-Verhalten unter Druck, 9-Risikoentwicklung, 10-Chancen (capped, modest when diverging), 11-Risiken (Zielverschiebung when diverging), 12-Handlungsempfehlung ("Abweichende Wirkung klar einordnen" first when diverging, "Strategische Wirkung offen einordnen" when person closer to goal), 13-Alternativwirkung. GoalContext computed via GOAL_TRIADS ideal profiles with `personDivergesFromGoal`, `personPrimary`, `teamPrimary` fields.
