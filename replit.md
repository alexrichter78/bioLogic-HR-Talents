...
-   **bioLogic JobCheck (MatchCheck)**: Assesses candidate-role fit, including dominance shift analysis and risk assessments. Uses a unified engine: `computeCoreFit()` in `jobcheck-engine.ts` is the single source of truth for fit rating (SUITABLE/CONDITIONAL/NOT_SUITABLE) and base control intensity. Accepts optional `externalKo` parameter so callers can pass KO from `koRuleTriggered`. Returns `CoreFitResult` with `overallFit`, `controlIntensity`, `mismatchScore`, `koTriggered`, `reasons: FitReason[]` (tracking which rules fired and their effect: KO/OVERRIDE/CAP/BASE), and `flags` (structural analysis: `sameDom`, `effectiveSameDom`, `roleIsBalFull`, `candIsBalFull`, `equalDistConflict`, `dualConflict`, `roleKeyInDual`, `secondaryFlipped`, `maxGapVal`, `candSpread`). `runEngine` delegates fit calculation to `computeCoreFit(role_profile, cand_profile, koRuleTriggered(...))` — no duplicated logic. `computeSollIst` likewise passes `koRuleTriggered` result as `externalKo` to `computeCoreFit`. `buildRoleAnalysisFromState()` is the shared builder function that constructs `RoleAnalysis` from stored RollenDNA state, used by both pages to ensure parity.
    -   **Part A/B Alignment**: Severity thresholds in Part B (Soll-Ist-Bericht) are aligned with Part A (Fit-Engine): 0–5 = ok, 6–10 = warning, >10 = critical. Impact Area labels: "Stimmig" / "Mit Abweichung" / "Kritisch". Master rule enforces that no Impact Area sounds more positive than the overall fit verdict. `fitGap` (not `devGap`) maps Fit-Label to development potential. **FitSubtype system**: `FitSubtype` (PERFECT | STRUCTURE_MATCH_INTENSITY_OFF | PARTIAL_MATCH | MISMATCH) derived from `StructureRelation` (EXACT | SOFT_CONFLICT | HARD_CONFLICT) drives differentiated text in all text builders (summary, dominance shift, executive bullets, constellation risks, impact areas, final text). PERFECT = "deckungsgleich"; STRUCTURE_MATCH_INTENSITY_OFF = "in der Grundrichtung stimmig, Gewichtung nicht deckungsgleich". Severity labels: ok="Weitgehend stimmig", warning="Mit Abweichung", critical="Kritisch". Words "passt"/"stimmt überein"/"deckungsgleich" reserved for PERFECT; "deutlich ab" forbidden unless maxGap≥15. Gesamtbewertung text uses fitSubtype to differentiate STRUCTURE_MATCH_INTENSITY_OFF ("Grundstruktur stimmig, Gewichtung leichte Abweichungen") from generic BEDINGT text.
...
### Region Switcher
-   **Context**: `client/src/lib/region.tsx` provides `RegionProvider` and `useRegion()` hook. Supports DE (Germany), CH (Switzerland), AT (Austria), **EN (English/international)**. Persisted in localStorage.
-   **Frontend**: Flag emoji dropdown (🇩🇪/🇨🇭/🇦🇹/🇬🇧) in `global-nav.tsx`. All pages with AI calls pass `region` in their API request body.
-   **Backend**: `getRegionInstruction(region, options?)` in `server/routes.ts` generates prompt instructions per region. CH: replaces ß→ss, uses Swiss terms. AT: uses Austrian terms. EN: generates English-language output. All default to formal "Sie" except KI-Coach which uses `skipAddress: true`.
...
-   **Deterministic text localization**: All engine/data files use "ss" as neutral base text. `ssToSz()` converts ss→ß for DE/AT. `localizeDeep<T>(obj, region)` recursively walks objects to localize all strings. Applied in: soll-ist-bericht, team-report, teamdynamik, jobcheck, teamcheck, rollen-dna, rollenprofil.
-   **EN UI Chrome Translation**: When region === "EN", all user-facing labels, headers, buttons, section titles, and status badges are rendered in English. Pattern: inline `region === "EN" ? "..." : "..."` conditionals throughout. Covered: jobcheck.tsx (StatusBadge, desc fields, "Critical area"), teamcheck.tsx (all section headers S1–S11, KPI tiles, exec summary, PDF button, tabs, metadata labels, tl.label badge), teamdynamik.tsx (page title, profile labels, role toggles, KPI tiles, tl.label badge, intensityLabel() helper), ki-coach.tsx (WELCOME_MSG_EN constant, isWelcomeMsg() helper, region-aware welcome on reset/region-swap), help-bot.tsx (full chrome: header, placeholder, escalation button, error messages), rollen-dna.tsx (regeneration banner). Task data values regenerated via Claude EN prompt on region switch — not literally translated.
-   **axisLabel(value, region)** in teamcheck.tsx already accepts optional region param for EN labels (hoch/mittel/gering → High/Medium/Low).
-   **translateEngineValue(value, region)** in `client/src/lib/region.tsx`: maps 25+ German engine output values (Spannung, Mittel, Ergänzung, Gesamtpassung labels, etc.) to English when region === "EN". Used in team-report.tsx and teamcheck.tsx for deterministic engine outputs stored as German strings in localStorage/engine results.
-   **roleResultTextsEN** in `client/src/pages/rollen-dna.tsx`: 13-entry EN equivalent of `roleResultTexts`, used by `getRoleResultEntry(key, region)` helper. `generateBioCheckText()` accepts region param and renders EN body text when region === "EN".
-   **team-report.tsx EN chrome**: All section headers, labels, badges, and metric titles are region-aware (inline `region === "EN" ? "..." : "..."` pattern). `severityLabel(s, region)` accepts optional region param (critical/conditional/fitting in EN). `gesamtpassungLabel` and `sw.label` passed through `translateEngineValue()`. Body text (managementSummary, teamstruktur, teamdynamikAlltag, kulturwirkung, systemwirkung, developmentText, fuehrungsprofil) remains German — full EN body text requires EN templates in team-report-engine.ts (future work).
-   **soll-ist-bericht.tsx EN chrome**: "Generate report" button, shortFazit/devShort variants, kritischLabel (Strengths/Notable/Critical), kritischBullets, auswirkungBullets all region-aware.
...
-   **Strukturanalyse / Entscheidungsbericht**: Features a dark anthrazit header, specific badge displays, and a structured body with colored section bars. All deterministic report text is generated by `generateJobCheckRoleReport()` in `client/src/lib/entscheidungsbericht-engine.ts`. The engine accepts a `JobCheckTextInput` (jobTitle, tasks, triad, successFocus, environment) and returns a `JobCheckReportTexts` object with 15+ output fields (intro, shortDescription, structureProfile, componentMeaning, workLogic, framework, successFocus, behaviourDaily/Pressure/Stress, teamImpact, tensionFields, miscastRisks, typicalPerson, finalDecision). Supports 13 structural variants via `VariantType` and 4 profile classes (BAL_FULL, DUAL_TOP, CLEAR_TOP, ORDER). `buildProfilkonflikt` remains in `rollenprofil.tsx` as it requires haupt-profile data outside the engine's scope.
