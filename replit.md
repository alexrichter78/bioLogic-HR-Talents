...
-   **Enterprise Organizations**: `organizations` table with name, optional `aiRequestLimit`, `aiRequestsUsed` counter, and `currentPeriodStart`. Users can be assigned to an org via `organizationId` FK on `users`. Admin CRUD at `/api/admin/organizations`. Usage reset at `/api/admin/organizations/:id/reset-usage`.
-   **Usage Tracking**: `usage_events` table logs every AI call (ki_coach, generate_bericht, generate_analyse) with userId, organizationId, eventType. Tracked fire-and-forget after successful responses. Admin can query org usage at `/api/admin/organizations/:id/usage?since=`. Subadmins can query their own org at `/api/subadmin/usage`.
-   **KI Limit Enforcement**: Before any AI call (ki-coach, generate-bericht, generate-analyse), `checkOrgAiLimit()` verifies the user's org hasn't exceeded its `aiRequestLimit`. If exceeded, returns a German-language error message. Orgs with `aiRequestLimit=null` have unlimited access.
-   **Subadmin Role**: `requireSubadmin` middleware allows both admin and subadmin roles. Subadmins see their own org's usage stats via `/api/subadmin/usage`.
