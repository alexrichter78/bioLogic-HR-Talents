...
-   **bioLogic JobCheck (MatchCheck)**: Assesses candidate-role fit, including dominance shift analysis and risk assessments. Uses a unified engine: `computeCoreFit()` in `jobcheck-engine.ts` is the single source of truth for fit rating (SUITABLE/CONDITIONAL/NOT_SUITABLE) and control intensity, used by both the MatchCheck page (`jobcheck.tsx` via `runEngine`) and the Soll-Ist-Bericht (`soll-ist-bericht.tsx` via `computeSollIst` which delegates to `computeCoreFit`).
