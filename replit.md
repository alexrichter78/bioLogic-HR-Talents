...
-   **Customizable Dashboard Widgets**: The home page (`client/src/pages/home.tsx`) features a configurable widget dashboard. Users can add/remove/rearrange 6 widget types: Schnellzugriff (quick actions), Coach-Aktivität (stats: conversations, feedback, satisfaction rate, top topics), Letzte Themen (recent coaching topics), Analyse-Status (current role analysis progress from localStorage), bioLogic-Impuls (rotating daily tips), Team-Übersicht (team triad percentages). Widget configuration (order + visibility) persists in localStorage. Configuration panel accessible via "Anpassen" button with up/down reorder and eye toggle. Backend API: `GET /api/dashboard-stats` returns per-user coach topic/feedback statistics via `storage.getDashboardStats(userId)`. Widgets handle logged-in, not-logged-in, error, and loading states distinctly.
