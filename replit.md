...
-   **bioLogic JobCheck (MatchCheck)**: Assesses candidate-role fit, including dominance shift analysis and risk assessments. Uses a unified engine: `computeCoreFit()` in `jobcheck-engine.ts` is the single source of truth for fit rating (SUITABLE/CONDITIONAL/NOT_SUITABLE) and control intensity. Both the MatchCheck page (`jobcheck.tsx` via `runEngine`) and the Soll-Ist-Bericht (`soll-ist-bericht.tsx` via `computeSollIst`) use the same KO rules (`koRuleTriggered`) and control intensity calculation (`calcControlIntensity`) when a full `RoleAnalysis` object is available. `buildRoleAnalysisFromState()` is the shared builder function that constructs `RoleAnalysis` from stored RollenDNA state, used by both pages to ensure parity.
