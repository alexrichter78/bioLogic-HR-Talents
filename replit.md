...
-   **JobCheck Baseline-Korrektur (Apr 2026, finale Version)**: Bereichswerte für Tätigkeiten / Humankompetenzen / Rahmenbedingungen / Führungskompetenzen werden in `client/src/lib/jobcheck-engine.ts` (`calcBG`, `calcRahmen`) mit der 33,33 %-Baseline gemischt (Mischfaktor `JOBCHECK_BASELINE_MIX = 0.5`, exportiert). So liegen Soll-Werte im realistischen Korridor (max ~50 %) statt im theoretischen 0–100 %. **Symmetrische Behandlung Stelle/Person**: In `runEngine()` wird das Personen-Profil einmalig durch `applyJobcheckBaseline()` auf dieselbe Match-Skala gebracht wie das (bereits in `calcBG`/`calcRahmen` gestauchte) Stellen-Profil — der Vergleich passiert damit symmetrisch, das Personen-Profil im UI bleibt mit den vom User eingegebenen Roh-Werten sichtbar. **Match-Toleranzen** in `computeCoreFit()`: `EQ_TOL = 3` (skaliert zur komprimierten Skala), `GOOD_TOL = 5`, `COND_TOL = 10` (Standard-Werte). Toleranzen sind über den optionalen `tolerances`-Parameter konfigurierbar. Dominanz-Reihenfolge der Bereiche bleibt mathematisch erhalten. Validiert mit `scripts/jobcheck-baseline-test.ts` (8 Stellen × 8 Personen, V3a_skaliertEQ als Sieger mit 26/64 Treffern, 12/12 NICHT-Geeignet-Erkennung perfekt).
